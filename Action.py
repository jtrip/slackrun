...
def runner(action_info):
    ...
    description = ''
    description += ('_' + character['Name'] + ' ' + action_info['action_name'] + '_\n')

    description += (
        action_info['attribute'] + "(" + str(character[action_info['attribute']]) + ")  + " +
        action_info['skill'] + "(" + str(character[action_info['skill']]) + ") + ")
    ...
    for i, key in enumerate(action_info['modifiers']):
        if i == last:
            ...
            operator = '+ '
        description += (
            str(key) + "(" + str(action_info['modifiers'][key]) + ") " + operator)
